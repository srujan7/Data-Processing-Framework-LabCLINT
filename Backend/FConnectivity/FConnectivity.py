        ...
        for x in range(
            ...
            subjects_of_each_condition[each_condition].append(
                ConSubject("%s %s" % (conditions_list[each_condition], (str)(
                    x + 1))))  # Create a new instance of class ConSubject
            while True:
                try:
                    ...
                    file_Location = input(
                        "\nPlease try again. \n")  # request user to input path to data file
                    fileName = data_folder / file_Location  # compile full path of data file
...
def export_to_excel():
    ...
    for organized in range(len(conditions_list)):
        data_organized.append(xlsxwriter.Workbook("%s_Organized_FuncConn.xlsx" % conditions_list[organized]))
        sheets.append([])
...
def makeGraphs():
    ...
    if valid_num_of_graphs:
        for graphs in range(len(num_of_graphs)):
            ...
            plt.savefig('FuncConn ROIs %s.png' % num_of_graphs[graphs])
